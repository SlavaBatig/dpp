...
const calculate = data => {
  ...
  for (const id in HEROES) {
    ...
    data.forEach(set => {
      ...
      totalExp += isTurbo ? (games + wins) * Constants.EXP_TURBO : (games + wins) * Constants.EXP_NORMAL
      totalExp += isTurbo ? (wins / 4) * (Constants.EXP_TURBO * 10) : (wins / 4) * (Constants.EXP_NORMAL * 10)
    })
